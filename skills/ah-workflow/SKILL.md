...
phase (except the CLAUDE.md revision in phase 5, which runs in the main session) -- carrying the right
...
- **Phase skills don't need a committer here**: each `ah-*` phase skill already runs its
  ...
  is phase 5 (`revise-claude-md`), which is not an `ah-*` skill and does not commit -- the orchestrator
  must commit its CLAUDE.md edits itself, before phase 6 creates the PR (see phase 5).
...
| 5 | `claude-md-management:revise-claude-md` | the current session | CLAUDE.md updated with learnings (committed onto the feature branch) |
| 6 | `ah-finalize-code` | reads spec.md metadata | simplify + tests + docs + review, then creates/updates the PR via `ah-create-pr` (the PR includes phase 5's CLAUDE.md commit) |

`ah-create-pr` is **not** a separate phase -- `ah-finalize-code` calls it at the end of phase 6.
Phase 5 (`revise-claude-md`) is placed **before** phase 6 deliberately, so the CLAUDE.md learnings
land on the branch and ship as part of the PR rather than as an uncommitted change after it.
...
```
/goal the ah-workflow progress file for issue <N> shows all six phases complete (PRD+ADR, tasks, implement, QA, claude-md, finalize+PR), or stop after <T> turns
```
...
2. **Launch a subagent** (Opus, low) whose prompt is: "Invoke the skill `<ah-skill-for-phase-k>`
   ...
   status." **Exception -- phase 5 runs in the main session, not a subagent** (see its bullet below).
   ...
   - Phases 3, 6: nothing extra -- the skill reads `spec.md`. Just name the spec dir if helpful.
   - Phase 4: the detected dev-server URL.
   - Phase 5: do **not** spawn a subagent. Invoke `claude-md-management:revise-claude-md` in the main
     session, because it reflects on "this session" -- a subagent would only see its own empty context,
     not the whole pipeline you just ran. It has no internal committer (it's not an `ah-*` skill), and
     it runs **before** phase 6 creates the PR, so commit its CLAUDE.md edits onto the feature branch
     yourself (e.g. `git add -A && git commit -m "docs: update CLAUDE.md with session learnings"`) so
     phase 6's PR ships them. If it made no changes, note that and move on.
...
- **Critical findings** -> report them and **pause the workflow before phase 6 (finalize)** so the user
...
phase, paths to the PRD / ADR / spec dir / QA report, a note on the CLAUDE.md revision from phase 5,
and the PR URL from phase 6. Echo the full progress file one last time so the `/goal` evaluator can
